...
3. Vector отбрасывает не-JSON строки, оставляет только события уровней `WARN`, `ERROR`, `FATAL`, добавляет метаданные источника и отправляет события в Kafka `errlog-core`-контура.
...
В таблице учтены только активные тесты. Дополнительно есть 2 отключённых smoke-теста (`*ApplicationTests`, требуют полный Spring-контекст + живую инфраструктуру) — итого 224 теста в проекте.
...
cd errapi && ./mvnw verify
```

Требуется JDK 21. Интеграционные тесты требуют работающий Docker.
