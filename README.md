...
```bash
curl -sS   -H "Authorization: Bearer $ERRLOG_OWNER_JWT"   -H "Content-Type: application/json"   -X POST "http://localhost:8080/api/errors/events?limit=10&offset=0"   -d '{}'
```

С явными границами времени:

```bash
curl -sS   -H "Authorization: Bearer $ERRLOG_OWNER_JWT"   -H "Content-Type: application/json"   -X POST "http://localhost:8080/api/errors/events?limit=20&offset=0"   -d '{"from":"2026-02-24T00:00:00Z","to":"2026-02-25T00:00:00Z"}'
```

Пример с фильтрами:

```bash
curl -sS   -H "Authorization: Bearer $ERRLOG_OWNER_JWT"   -H "Content-Type: application/json"   -X POST "http://localhost:8080/api/errors/events?limit=20&offset=0"   -d '{"to":"2026-02-25T00:00:00Z","filters":[{"field":"service","operation":"eq","values":["jerrgen"]},{"field":"level","operation":"in","values":["ERROR"]}]}'
```

### 3. Детальная информация по событию

```bash
curl -sS   -H "Authorization: Bearer $ERRLOG_OWNER_JWT"   "http://localhost:8080/api/errors/events/<eventId>"
```

### 4. Группы ошибок

```bash
curl -sS   -H "Authorization: Bearer $ERRLOG_OWNER_JWT"   -H "Content-Type: application/json"   -X POST "http://localhost:8080/api/errors/groups?limit=10&offset=0"   -d '{}'
```
...
```bash
curl -sS   -H "Authorization: Bearer $ERRLOG_OWNER_JWT"   -H "Content-Type: application/json"   -X POST "http://localhost:8080/api/errors/timeseries"   -d '{}'
```

Явный бакет:

```bash
curl -sS   -H "Authorization: Bearer $ERRLOG_OWNER_JWT"   -H "Content-Type: application/json"   -X POST "http://localhost:8080/api/errors/timeseries?bucket=15m"   -d '{}'
```
