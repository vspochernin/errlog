...
Проект полезен в ситуациях, когда приложение уже пишет логи, а команде нужно не просто просматривать отдельные строки журналов, а работать с ошибками как с отдельными проблемами: видеть группы однотипных событий, открывать конкретные события, смотреть детали ошибки и анализировать динамику во времени.
...
- `errlog-demo` - демонстрационный контур: Jerrgen и Vector.
  - Этот контур нужен для демонстрации работы сервиса и имитирует внешние микросервисы, генерирующие ошибки в процессе своей работы.
