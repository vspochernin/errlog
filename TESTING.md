...
| `TimeWindowParserTest` | 9 | from/to заданы, null->now, null->now-24ч, from>to, from==to (граница), невалидный формат, пустые строки |
| `ValidationUtilsTest` | 7 | limit [1,500], offset>=0, валидный/невалидный UUID |
| `ErrorsWhereBuilderTest` | 8 | Базовый WHERE, fingerprint, EQ/NE/IN/LIKE, columnPrefix (timestamp + фильтры), несколько фильтров (точные имена параметров) |
| `TimeBucketTest` | 13 | `byName` (все 6) + неизвестный, `byTimeWindow` (границы 1h/6h/24h/7d/30d ровно на `<=` + over 30d) |
...
| `ErrorsAllowlistTest` | 4 | `byName` - существует/не существует, level без LIKE (с поддержкой EQ/NE/IN), snake_case маппинги (sourceType->source_type и др.) |
| `ErrorsQueryTest` | 2 | parseFromErrorsRequest - полный (со значениями fingerprint/фильтров), пустой |
| `UserRoleTest` | 11 | OWNER меняет всё кроме OWNER, ADMIN меняет READER/NONE, READER меняет NONE, запреты (change vs assign различимы), NONE бесправен |
...
| Класс | Тестов | Что проверяет |
|---|---|---|
| `ErrorsServiceTest` | 9 | getFilters, getEvents (успех, невалидный limit), getEventById (успех с проверкой полей, UUID, not found), getGroups, getTimeseries (авто/ручной бакет) |
| `UserServiceTest` | 6 | listUsers, getUser (найден/не найден), changeRole (успех + verify(save), своя роль, READER не может повысить NONE) |
| `AuthServiceTest` | 8 | register (успех, login exists, email exists), login (успех, bad credentials), changePassword (успех, старый не совпадает, новый==старый) |
...
| Класс | Тестов | Что проверяет |
|---|---|---|
| `ErrorsRepositoryIT` | 8 | Реальные SQL к ClickHouse: countEvents, findEvents (пагинация с проверкой пересечения страниц), findEventById (найден/не найден), countEventsAndGroupsTotals, findGroups (count/lastEvent/сортировка), findTimeseries (сумма counts + сортировка) |
| `UserRepositoryIT` | 5 | Реальная БД на PostgreSQL + Flyway: создание, уникальность логина (DuplicateKeyException), CHECK-ограничение роли (SQLSTATE 23514), existsByRole (точный count) |
